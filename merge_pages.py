"""
Merge per-page transcriptions from pages/txt/ into ch/.

Three-phase approach:
1. Build front-matter files (title page, "To the Reader", "The Argument")
   from pages 7–14.
2. Concatenate all dialogue pages into ch/all.md, stripping headers and
   catch-words and joining text that flows across page boundaries.
3. Split ch/all.md into individual chapter files (ch/1.md–ch/10.md) by
   finding lines that say "Dialogue I.", "Dialogue II.", etc.

This script was generated by Claude Opus 4.6
"""
...
PAGES_DIR = os.path.join(BASE, "pages/txt")
CH_DIR = os.path.join(BASE, "ch")

# Page ranges
FIRST_PAGE = 15   # First dialogue page
LAST_PAGE = 309   # Last dialogue page

# Front-matter sections: (output filename, title, nav_order, page ranges)
FRONT_MATTER = [
    ("title-page.md", "Title Page", -3, [7]),
    ("to-the-reader.md", "To the Reader", -2, [9, 10]),
    ("the-argument.md", "The Argument", -1, [13, 14]),
]
...
# --- Phase 1: Build front-matter files ---

print("Phase 1: Building front-matter files...")

os.makedirs(CH_DIR, exist_ok=True)

for fname, title, nav_order, page_nums in FRONT_MATTER:
    fm_pages = []
    for scan in page_nums:
        raw = read_page(scan)
        if raw is None:
            continue
    ...
    body = join_pages([p for p in fm_pages if p])
...
print("Phase 3: Splitting into individual chapter files...")
